...
export async function findCardsByEmployeeIdAndPasswords(
  employeeId: number,
  passwords: [string]
) {
...
function setExpirationDate() {
  ...
  const expirationDate = `${padStartZero(currentMonth)}/${padStartZero(
    expiryYear
  )}`;
...
function setSecuritCodeCard() {
  const securityCode = internalCryptr.encrypt(faker.finance.creditCardCVV());
...
async function handleListCardRequest(employeeId: number, passwords: [string]) {
  ...
  const cardsResponse = await cardRepository.findActiveCardByEmployeeId(
    employeeId
  );
...
export async function findEmployee(cpf: string, companyId: number) {
  const employee = await employeeRepository.findByCompanyIdAndCPF(
    cpf,
    companyId
  );
...
async function isDifferentTypeCard(type: TransactionTypes, employeeId: number) {
  const findCardSameType = await cardRepository.findByTypeAndEmployeeId(
    type,
    employeeId
  );
  if (findCardSameType) {
